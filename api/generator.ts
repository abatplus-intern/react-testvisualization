...
let urlBase = `https://XXX.app.corpintra.net`;
//TODO: here you need to set your default service name
let service = 'api';
let serviceVersion = 'v1';
const defaultUrlSheme = `${urlBase}/${service}/${serviceVersion}/docs`;

type HttpIncomingMessageCallback = (res: http.IncomingMessage) => void;

async function downloadApiDefinition(host: string, path: string, targetPath: string, useTLS = true) {
    console.log(`Downloading api definition from '${host}${path}' ...`);
    ...
    if (useTLS)
        response = await new Promise<http.ClientRequest>((resolve) =>
            https.get(
                ...
                resolve as unknown as HttpIncomingMessageCallback
            )
        );
    else
        response = await new Promise<http.ClientRequest>((resolve) =>
    ...
    console.log(chalk.yellow(`Saving api definition to '${targetPath}' ...`));
...
function runOpenApiGenerator(tempPath = workDir, tempSwaggerFile = swaggerDefinitionFile) {
    ...
    console.log(chalk.green('open-api-generator DONE!'));
}
...
async function hasPatchFileAnApiChange(file: string, patchFile: string): Promise<boolean> {
    ...
    if (fileStats.size === 0) {
        ...
        console.log(chalk.magenta(`"${file}" is a new API file API`));
        return true;
    }
...
async function apiInFileChanged(file: string): Promise<boolean> {
    console.log(chalk.yellow(`Checking for api changes in "${file}"...`));
    ...
    if (apiChange) console.log(chalk.magenta(`"${file}" has API changes.`));
    else console.log(chalk.green(`"${file}" has NO api changes.`));
    return apiChange;
}
...
function gitRestoreFile(file: string) {
    ...
    console.log(chalk.greenBright(`restored "${file}".`));
}
...
async function validateApiFilesForChanges(folder: string) {
    console.log(chalk.yellow(`validating files for real api changes...`));
    ...
    console.log(chalk.green(`validating DONE! (you can now commit new and changed api files)`));
}

async function main(url: string, serviceName: string) {
    console.log(chalk.yellow('Starting api interface generation...'));
    // Cleanup old files
    ...
    deleteFolderRecursive(workDir);
    deleteFile(swaggerDefinitionFile);
    console.log(chalk.green(`!!! api interface generation for service "${serviceName}" DONE !!!`));
